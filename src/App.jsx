...
const DEPTS = [
  { id:"hq",      name:"NOVI HQ",             notionName:"HQ",             icon:"🏛️", color:"#0066FF", chatUrl:"https://claude.ai/chat/a7dad3eb-def3-4631-a991-20ff81cfae01" },
  { id:"coo",     name:"NOVI COO",            notionName:"COO",            icon:"👔", color:"#00B07D", chatUrl:"https://claude.ai/chat/6ec486de-12f5-4007-9025-0eb01c10ff0e" },
  { id:"cfo",     name:"NOVI CFO",            notionName:"CFO",            icon:"💰", color:"#F59E0B", chatUrl:"https://claude.ai/chat/c420d1fa-ec99-4953-ab43-e2b47b4924df" },
  { id:"cmo",     name:"NOVI CMO",            notionName:"CMO",            icon:"📣", color:"#EC4899", chatUrl:"https://claude.ai/chat/d44df1ba-76ae-464a-91ef-2c6d708e0bc4" },
  { id:"clo",     name:"NOVI CLO",            notionName:"CLO",            icon:"⚖️",  color:"#8B5CF6", chatUrl:"https://claude.ai/chat/2cca67c7-ec65-46c4-9db1-819b224d8b87" },
  { id:"form",    name:"Formulations",        notionName:"Formulations",   icon:"🧪", color:"#06B6D4", chatUrl:"https://claude.ai/chat/302e1fd9-d88a-4e99-9020-620b23ebae8c" },
  { id:"mkt",     name:"Market Strategy",     notionName:"Market Strategy",icon:"🎯", color:"#10B981", chatUrl:"https://claude.ai/chat/c704fffd-2165-48b3-b433-bc9b0dbee7a1" },
  { id:"ops",     name:"Operations",          notionName:"Operations",     icon:"⚙️",  color:"#3B82F6", chatUrl:null },
  { id:"vendors", name:"Vendors",             notionName:"Vendors",        icon:"🤝", color:"#F97316", chatUrl:null },
  { id:"comply",  name:"Compliance",          notionName:"Compliance",     icon:"📋", color:"#EAB308", chatUrl:null },
];
...
const Ic = ({ n, s=16 }) => {
  const m = {
    ...
    bell  :<svg width={s} height={s} fill="none" stroke="currentColor" strokeWidth="1.75" viewBox="0 0 24 24"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>,
...
const PriBadge = ({ p }) => {
  const cfg = { Critical:["#FEE2E2","#991B1B"], High:["#FFEDD5","#9A3412"], Medium:["#FEF3C7","#92400E"], Low:["#F3F4F6","#6B7280"] };
  const [bg,fg] = cfg[p] || ["#F3F4F6","#6B7280"];
  return <Badge label={p} bg={bg} fg={fg}/>;
};
...
const Input = ({ value, onChange, placeholder, type="text" }) => (
  <input type={type} value={value} onChange={onChange} placeholder={placeholder}
    style={{ width:"100%", border:"1px solid #E5E7EB", borderRadius:8, padding:"9px 12px", fontSize:13, color:"#111827", background:"#fff", fontFamily:"inherit", outline:"none", transition:"border .15s" }}
...
const Textarea = ({ value, onChange, placeholder, rows=4 }) => (
  <textarea value={value} onChange={onChange} placeholder={placeholder} rows={rows}
    style={{ width:"100%", border:"1px solid #E5E7EB", borderRadius:8, padding:"9px 12px", fontSize:13, color:"#111827", background:"#fff", fontFamily:"inherit", outline:"none", resize:"vertical", transition:"border .15s" }}
    onFocus={e=>e.target.style.borderColor="#6366F1"}
    onBlur={e=>e.target.style.borderColor="#E5E7EB"} />
);
...
const SectionTitle = ({ children }) => (
  <h2 style={{ fontSize:18, fontWeight:700, color:"#111827", marginBottom:4 }}>{children}</h2>
);

const SectionSub = ({ children }) => (
  <p style={{ fontSize:13, color:"#6B7280", marginBottom:20 }}>{children}</p>
);
...
export default function NOVIApp() {
  ...
  const NavItems = () => (
    <>
      {nav.map(item => (
        <button key={item.id}
          onClick={() => { setTab(item.id); setMN(false); }}
          style={{ width:"100%", display:"flex", alignItems:"center", gap:10, padding:"10px 14px", borderRadius:8, border:"none", background: tab===item.id ? "#EEF2FF" : "transparent", color: tab===item.id ? "#4F46E5" : "#6B7280", fontSize:14, fontWeight: tab===item.id ? 600 : 400, cursor:"pointer", textAlign:"left", fontFamily:"inherit", transition:"all .15s", marginBottom:2 }}>
          <span style={{ color: tab===item.id ? "#4F46E5" : "#9CA3AF" }}><Ic n={item.icon} s={17}/></span>
          {item.label}
      ...
      <div style={{ padding:"16px 14px 6px", fontSize:11, fontWeight:600, color:"#9CA3AF", letterSpacing:1, textTransform:"uppercase" }}>Department Chats</div>
      {DEPTS.filter(d=>d.chatUrl).map(d => (
        <a key={d.id} href={d.chatUrl} target="_blank" rel="noreferrer"
          style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 14px", borderRadius:8, color:"#6B7280", fontSize:13, textDecoration:"none", fontWeight:400, transition:"all .15s", marginBottom:2 }}
          onMouseEnter={e=>{ e.currentTarget.style.background="#F9FAFB"; e.currentTarget.style.color="#111827"; }}
          onMouseLeave={e=>{ e.currentTarget.style.background="transparent"; e.currentTarget.style.color="#6B7280"; }}>
          <span style={{ fontSize:16 }}>{d.icon}</span>
          <span>{d.name}</span>
          <span style={{ marginLeft:"auto", color:"#D1D5DB" }}><Ic n="ext" s={12}/></span>
        </a>
      ))}
    </>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#F9FAFB", fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif", color:"#111827" }}>
      ...
      <aside className="desktop-sidebar" style={{ position:"fixed", top:0, left:0, width:240, height:"100vh", background:"#fff", borderRight:"1px solid #E5E7EB", display:"flex", flexDirection:"column", zIndex:100, overflowY:"auto" }}>
        ...
        {/* Bottom actions */}
        <div style={{ padding:"12px 16px 20px", borderTop:"1px solid #F3F4F6", display:"flex", flexDirection:"column", gap:8 }}>
          <button onClick={triggerEOD}
            style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6, padding:"10px", background:"#111827", color:"#fff", border:"none", borderRadius:8, fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>
      ...
      {/* ── MOBILE NAV OVERLAY ───────────────────────────── */}
      {mobileNav && (
        <div className="mobile-nav-overlay" style={{ position:"fixed", inset:0, zIndex:200 }}>
          ...
          <div style={{ position:"absolute", top:0, left:0, width:280, height:"100vh", background:"#fff", overflowY:"auto", padding:"16px 10px" }}>
            ...
            <NavItems/>
            <div style={{ padding:"16px 14px 0", borderTop:"1px solid #F3F4F6", marginTop:8, display:"flex", flexDirection:"column", gap:8 }}>
              <button onClick={()=>{triggerEOD();setMN(false);}}
                style={{ padding:"10px", background:"#111827", color:"#fff", border:"none", borderRadius:8, fontSize:13, fontWeight:600, cursor:"pointer" }}>
                Send EOD Report
              </button>
            </div>
          </div>
      ...
      <main className="main-content" style={{ marginLeft:240, minHeight:"100vh" }}>
        {/* Page header */}
        <div style={{ background:"#fff", borderBottom:"1px solid #E5E7EB", padding:"16px 32px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:50 }}>
          <div>
            <h1 style={{ fontSize:16, fontWeight:700, color:"#111827" }}>{nav.find(n=>n.id===tab)?.label}</h1>
            <p style={{ fontSize:12, color:"#9CA3AF", marginTop:1 }}>{new Date().toLocaleDateString("en-IN",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</p>
          </div>
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            {founderReq.length>0 && (
              <span style={{ background:"#FEF3C7", color:"#92400E", fontSize:12, borderRadius:6, padding:"4px 10px", fontWeight:500 }}>
                ⚠️ {founderReq.length} input needed
              </span>
            )}
        ...
        <div style={{ padding:"28px 32px", maxWidth:1100 }}>
          {loading
            ...
            : <div className="fade">
                {tab==="dashboard" && <DashView tasks={tasks} updates={updates} decisions={decisions} urgent={urgent} blocked={blocked} open={open} pendDec={pendDec} founderReq={founderReq} setTab={setTab} pCol={pCol} sBg={sBg} sFg={sFg}/>}
                {tab==="sync"      && <SyncView depts={DEPTS} toast={toast} load={load}/>}
                {tab==="tasks"     && <TasksView tasks={tasks} toast={toast} load={load} pCol={pCol} sBg={sBg} sFg={sFg}/>}
                {tab==="decisions" && <DecisionsView decisions={decisions} pendDec={pendDec} toast={toast} load={load} aFg={aFg} aBg={aBg}/>}
      ...
      {/* Mobile padding */}
      <style>{`@media(max-width:768px){main{margin-left:0!important;padding-top:56px;}div[style*="padding:28px 32px"]{padding:20px 16px!important;}}`}</style>

      {/* Notification */}
      {notif && (
        <div className="notif" style={{ position:"fixed", bottom:24, right:24, background: notif.t==="ok" ? "#111827" : "#EF4444", color:"#fff", padding:"12px 20px", borderRadius:10, fontSize:13, fontWeight:500, zIndex:999, boxShadow:"0 4px 16px rgba(0,0,0,0.15)", maxWidth:320 }}>
          {notif.msg}
        </div>
      )}
    </div>
...
// ── DASHBOARD ─────────────────────────────────────────────────────────────────
function DashView({ tasks, updates, decisions, urgent, blocked, open, pendDec, founderReq, setTab, pCol, sBg, sFg }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom:28 }}>
        <h1 style={{ fontSize:26, fontWeight:800, color:"#111827", marginBottom:4 }}>{greeting}, Jaymin 👋</h1>
        <p style={{ fontSize:14, color:"#6B7280" }}>Here's your NOVI company status for today.</p>
      </div>

      {/* Alert bar */}
      {(urgent.length>0 || founderReq.length>0 || pendDec.length>0) && (
        <div style={{ background:"#FFFBEB", border:"1px solid #FCD34D", borderRadius:10, padding:"14px 18px", marginBottom:24, display:"flex", flexWrap:"wrap", gap:8, alignItems:"center" }}>
          <span style={{ fontSize:13, fontWeight:600, color:"#92400E" }}>⚠️ Needs your attention:</span>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:16, marginBottom:28 }}>
        {[
          { l:"Critical Tasks",    v:urgent.length,   sub:"Needs immediate action", color:"#EF4444", bg:"#FEF2F2", t:"tasks" },
          { l:"Blocked Tasks",     v:blocked.length,  sub:"Needs unblocking",        color:"#F97316", bg:"#FFF7ED", t:"tasks" },
          { l:"Open Tasks",        v:open.length,     sub:"Across all departments",  color:"#3B82F6", bg:"#EFF6FF", t:"tasks" },
          { l:"Pending Decisions", v:pendDec.length,  sub:"Awaiting your approval",  color:"#8B5CF6", bg:"#F5F3FF", t:"decisions" },
        ].map(s => (
          <div key={s.l} onClick={() => setTab(s.t)}
            style={{ background:"#fff", border:"1px solid #E5E7EB", borderRadius:12, padding:"20px 22px", cursor:"pointer", transition:"all .15s", borderLeft:`4px solid ${s.color}` }}
            onMouseEnter={e=>e.currentTarget.style.boxShadow="0 4px 12px rgba(0,0,0,0.08)"}
            onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
            <div style={{ fontSize:32, fontWeight:800, color:s.color, lineHeight:1, marginBottom:6 }}>{s.v}</div>
            <div style={{ fontSize:14, fontWeight:600, color:"#111827", marginBottom:2 }}>{s.l}</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))", gap:20, marginBottom:20 }}>
        ...
        <Card>
          ...
          {urgent.length===0
            ? <div style={{ textAlign:"center", padding:"24px 0", color:"#9CA3AF", fontSize:13 }}>No critical tasks 🎉</div>
            : urgent.slice(0,4).map(t => (
              <div key={t._id} style={{ padding:"10px 0", borderBottom:"1px solid #F3F4F6" }}>
                <div style={{ fontSize:13, fontWeight:500, color:"#111827", marginBottom:4 }}>{t["Task Name"]||"Untitled"}</div>
                <div style={{ display:"flex", gap:6, alignItems:"center", flexWrap:"wrap" }}>
                  <span style={{ fontSize:11, color:"#6B7280" }}>{t.Department}</span>
              ...
              return (
                <div key={d._id} style={{ padding:"12px 0", borderBottom:"1px solid #F3F4F6", display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:12 }}>
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:12, color:"#6B7280" }}>{(d["Decision Summary"]||"").slice(0,80)}</div>
...
function SyncView({ depts, toast, load }) {
  ...
  const handleSync = async () => {
    ...
    const r = await notionApi("syncDeptUpdate", {
      ...
      risks, founderDecisionRequired:fr, nextActions:nextAct,
    });
  ...
  return (
    <div style={{ maxWidth:680 }}>
      ...
      <SectionSub>After your Claude conversation, ask NOVI to summarise it, then paste below and sync to Notion in one click.</SectionSub>

      {/* How to */}
      <Card style={{ marginBottom:24, background:"#F0FDF4", borderColor:"#BBF7D0" }}>
        <h3 style={{ fontSize:13, fontWeight:700, color:"#065F46", marginBottom:10 }}>How to use</h3>
        {["1. Finish your conversation in any NOVI department chat",
          "2. Type: \"Summarise this conversation for HQ sync — include decisions, next actions, risks, founder input needed\"",
          "3. Copy Claude's output → paste below → select department → click Sync"
        ].map((s,i) => <p key={i} style={{ fontSize:12, color:"#047857", marginBottom:4, lineHeight:1.6 }}>{s}</p>)}
      </Card>
      ...
      <Card>
        ...
        <Field label="Summary (paste from Claude chat)">
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
          <Field label="Status"><Select value={status} onChange={e=>setStatus(e.target.value)} options={DEPT_STATUS}/></Field>
          <Field label="Priority"><Select value={priority} onChange={e=>setPri(e.target.value)} options={PRIORITY}/></Field>
        </div>
        ...
        <Btn onClick={handleSync} disabled={syncing||!summary.trim()} style={{ width:"100%" }}>
...
// ── TASKS VIEW ────────────────────────────────────────────────────────────────
function TasksView({ tasks, toast, load, pCol, sBg, sFg }) {
  ...
  const sections = [
    ...
    { key:"Low",      label:"Low",         color:"#9CA3AF", bg:"#F9FAFB" },
  ];
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:24, flexWrap:"wrap", gap:12 }}>
        <div>
          ...
          <SectionSub>{tasks.filter(t=>t.Status!=="Completed"&&t.Status!=="Deferred").length} open · {tasks.length} total · Live from Notion</SectionSub>
        </div>
      ...
      {form && (
        <Card style={{ marginBottom:24, borderColor:"#C7D2FE" }}>
          ...
          <Field label="Task Name"><Input value={form.taskName} onChange={e=>setForm(f=>({...f,taskName:e.target.value}))}/></Field>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:14, marginBottom:0 }}>
            <Field label="Department"><Select value={form.department} onChange={e=>setForm(f=>({...f,department:e.target.value}))} options={DEPTS.map(d=>d.notionName)}/></Field>
            <Field label="Priority"><Select value={form.priority} onChange={e=>setForm(f=>({...f,priority:e.target.value}))} options={PRIORITY}/></Field>
            <Field label="Status"><Select value={form.status} onChange={e=>setForm(f=>({...f,status:e.target.value}))} options={TASK_STATUS}/></Field>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            <Field label="Owner"><Input value={form.owner} onChange={e=>setForm(f=>({...f,owner:e.target.value}))}/></Field>
            <Field label="Deadline"><Input value={form.deadline} onChange={e=>setForm(f=>({...f,deadline:e.target.value}))} placeholder="YYYY-MM-DD"/></Field>
          </div>
          <Field label="Notes"><Input value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))}/></Field>
          <div style={{ display:"flex", gap:8 }}>
            <Btn onClick={create}>Create in Notion</Btn>
          ...
          <div key={sec.key} style={{ marginBottom:24 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {items.map(t => {
                ...
                return (
                  <div key={t._id} style={{ background:"#fff", border:"1px solid #E5E7EB", borderRadius:10, padding:"14px 18px", borderLeft:`4px solid ${pCol(t.Priority)}`, display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:12 }}>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:14, fontWeight:500, color:"#111827", marginBottom:6 }}>{t["Task Name"]||"Untitled"}</div>
                      <div style={{ display:"flex", gap:6, flexWrap:"wrap", alignItems:"center" }}>
                        ...
                        {t.Notes && <span style={{ fontSize:12, color:"#9CA3AF" }}>· {t.Notes.slice(0,40)}</span>}
                      </div>
                    </div>
                    <Badge label={t.Status||"—"} bg={sBg(t.Status)} fg={sFg(t.Status)}/>
                  </div>
      ...
      {tasks.filter(t=>t.Status==="Completed").length > 0 && (
        <div style={{ marginBottom:24 }}>
          ...
          {tasks.filter(t=>t.Status==="Completed").map(t => (
            <div key={t._id} style={{ background:"#F9FAFB", border:"1px solid #E5E7EB", borderRadius:10, padding:"12px 18px", marginBottom:6, opacity:.6 }}>
      ...
      {tasks.length===0 && !form && (
        <Card style={{ textAlign:"center", padding:"48px 24px" }}>
          <p style={{ color:"#9CA3AF", fontSize:14, marginBottom:8 }}>No tasks loaded from Notion yet.</p>
...
// ── DECISIONS VIEW ────────────────────────────────────────────────────────────
function DecisionsView({ decisions, pendDec, toast, load, aFg, aBg }) {
  const [form, setForm] = useState(null);
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6, flexWrap:"wrap", gap:12 }}>
        <div>
          ...
          <SectionSub>Institutional strategic memory · {decisions.length} total · {pendDec.length} pending approval</SectionSub>
        </div>
        <Btn onClick={()=>setForm({...empty})}><Ic n="plus" s={14}/> Log Decision</Btn>
      </div>

      <Card style={{ marginBottom:24, background:"#FAFAFA", borderColor:"#E5E7EB" }}>
        <p style={{ fontSize:13, color:"#6B7280", lineHeight:1.6 }}>
          📋 Every major decision is permanently stored here and synced to your Notion Decision Log. This is your company's institutional memory.
        </p>
      </Card>
      ...
      {form && (
        <Card style={{ marginBottom:24, borderColor:"#C7D2FE" }}>
          ...
          <Field label="Decision Title"><Input value={form.decisionTitle} onChange={e=>setForm(f=>({...f,decisionTitle:e.target.value}))}/></Field>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            <Field label="Department"><Select value={form.department} onChange={e=>setForm(f=>({...f,department:e.target.value}))} options={DEPTS.map(d=>d.notionName)}/></Field>
            <Field label="Founder Approval"><Select value={form.founderApproval} onChange={e=>setForm(f=>({...f,founderApproval:e.target.value}))} options={APPROVAL}/></Field>
          </div>
          <Field label="Decision Summary"><Textarea value={form.decisionSummary} onChange={e=>setForm(f=>({...f,decisionSummary:e.target.value}))} rows={3}/></Field>
          <Field label="Reasoning"><Textarea value={form.reasoning} onChange={e=>setForm(f=>({...f,reasoning:e.target.value}))} rows={3}/></Field>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            <Field label="Risks"><Input value={form.risks} onChange={e=>setForm(f=>({...f,risks:e.target.value}))}/></Field>
            <Field label="Long-Term Impact"><Input value={form.longTermImpact} onChange={e=>setForm(f=>({...f,longTermImpact:e.target.value}))}/></Field>
          </div>
          <div style={{ display:"flex", gap:8 }}>
            <Btn onClick={log}>Log to Notion</Btn>
            <Btn onClick={()=>setForm(null)} variant="outline">Cancel</Btn>
          </div>
        </Card>
      )}
      ...
      {groups.map(g => g.items.length > 0 && (
        <div key={g.key} style={{ marginBottom:28 }}>
          <h3 style={{ fontSize:13, fontWeight:700, color:"#374151", marginBottom:12, display:"flex", alignItems:"center", gap:8 }}>
            {g.label} <span style={{ fontSize:12, fontWeight:400, color:"#9CA3AF" }}>({g.items.length})</span>
          </h3>
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {g.items.map(d => {
              const dept = DEPTS.find(x=>x.notionName===d.Department);
              return (
                <Card key={d._id} style={{ borderLeft:`4px solid ${g.key==="Approved"?"#10B981":g.key==="Rejected"?"#EF4444":g.key==="Deferred"?"#9CA3AF":"#F59E0B"}` }}>
      ...
      {decisions.length===0 && !form && (
        <Card style={{ textAlign:"center", padding:"48px 24px" }}>
          <p style={{ color:"#9CA3AF", fontSize:14 }}>No decisions logged yet. Use the button above to log your first decision.</p>
        </Card>
      )}
    </div>
  );
...
function EODView({ triggerEOD, tasks, updates, urgent, open }) {
  ...
  return (
    <div style={{ maxWidth:680 }}>
      ...
      <SectionSub>Auto-sends to parekhjaymin11@gmail.com at 6:00 PM IST daily · Saved to Notion</SectionSub>
      ...
      <Card style={{ marginBottom:24, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:16 }}>
        <div>
          ...
          <div style={{ fontSize:13, color:"#6B7280", marginBottom:2 }}>Vercel cron runs at 12:30 UTC = 6:00 PM IST</div>
          <div style={{ fontSize:12, color:"#9CA3AF" }}>Report saved to Notion EOD Reports table after each send</div>
        </div>
        <Btn onClick={triggerEOD}><Ic n="eod" s={14}/> Send Now</Btn>
      </Card>

      {/* Today preview */}
      <h3 style={{ fontSize:14, fontWeight:700, color:"#374151", marginBottom:12 }}>Today's Report Preview</h3>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))", gap:12, marginBottom:24 }}>
        {[
          { l:"Critical",    v:urgent.length,                              c:"#EF4444", bg:"#FEF2F2" },
          { l:"Open Tasks",  v:open.length,                                c:"#3B82F6", bg:"#EFF6FF" },
          ...
          <div key={s.l} style={{ background:s.bg, borderRadius:10, padding:"16px", textAlign:"center", border:`1px solid ${s.c}20` }}>
            <div style={{ fontSize:28, fontWeight:800, color:s.c }}>{s.v}</div>
            <div style={{ fontSize:12, color:s.c, opacity:.8, marginTop:4 }}>{s.l}</div>
          </div>
        ))}
      </div>
      ...
      <Card>
        ...
        {["📊 Company snapshot with overall status",
          "⚠️ Founder attention required items",
          "🚨 Critical tasks with owner and deadline",
          "⚡ High priority tasks",
          ...
          <div key={i} style={{ padding:"8px 0", borderBottom:"1px solid #F3F4F6", fontSize:13, color:"#4B5563" }}>{item}</div>
        ))}
      </Card>
    </div>
  );
...
function DeptsView({ depts, updates }) {
  return (
    <div>
      ...
      <SectionSub>Click any department to open its Claude chat directly. After your conversation, sync the summary to Notion.</SectionSub>
